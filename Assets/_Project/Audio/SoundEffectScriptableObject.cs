...
namespace Kamikaze.Audio
{
	...
	public class SoundEffectScriptableObject : ScriptableObject
	{
		...
		[MinMaxRange(0, 1)] [SerializeField] private RangedFloat volume = new RangedFloat(0.5f, 0.5f);

		[MinMaxRange(0, 3)] [SerializeField] private RangedFloat pitch = new RangedFloat(1, 1);
		...
		{
			if (previewer != null) DestroyImmediate(previewer.gameObject);
		}
